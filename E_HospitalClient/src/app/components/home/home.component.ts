...
export class HomeComponent implements OnInit {
  ...
  getAllDoctors() {
    this.http.get("http://localhost:5056/api/Appointments/GetAllDoctors").subscribe((res: any) => {
  ...
  add(form: NgForm) {
    if (form.valid) {
      ...
      this.http.post("http://localhost:5056/api/Appointments/Create",data).subscribe(res=> {
  ...
  findPatientByIdentityNumber() {
    ...
    this.http.post<ResultModel<UserModel>>
      (`http://localhost:5056/api/Appointments/FindPatientByIdentityNumber`,
  ...
  onAppointmentDeleting(event: any) {
    ...
    if(result){
      ...
      this.http.get(`http://localhost:5056/api/Appointments/DeleteById?id=${id}`).subscribe(res=> {
        this.getDoctorAppointments();
      });
    }
  }
